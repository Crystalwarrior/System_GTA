//GTA version of daycycle. Not for use in your own things.
function getDayCycleTime() {
	...
	%time = $Sim::Time / DayCycle.dayLength + DayCycle.dayOffset + $GTA::DayCycle::TimeOffset;
...
function getDayCycleStageReal(%time) {
	%time = getDayCycleTimeString(%time);
	%time = strReplace(%time, ":", " ");
	if(getWord(%time, 0) >= 0) {
		%stage = 3;
	}
	if(getWord(%time, 0) >= $GTA::DayCycle::DawnTime) {
		%stage = 0;
	}
	if(getWord(%time, 0) >= $GTA::DayCycle::DayTime) {
		%stage = 1;
	}
	if(getWord(%time, 0) >= $GTA::DayCycle::DuskTime) {
		%stage = 2;
	}
	if(getWord(%time, 0) >= $GTA::DayCycle::NightTime) {
		%stage = 3;
	}
	return %stage;
}